...
def apply_effect(image_path):
    ...
    original_image_path = upload_dir / f"original_{image_path.name}"
    cv2.imwrite(str(original_image_path), cv2.cvtColor(image, cv2.COLOR_BGR2RGB))
    output_path = upload_dir / f"effect_{image_path.name}"
    cv2.imwrite(str(output_path), image)

    return original_image_path, output_path
    ...
    for unique_filename, display_filename in uploaded_files.items():
        ...
        images.append(
            ...
            <div style="display: inline-block; margin: 10px;">
                ...
                <br>
                <a style="background: none; border: none; font-size: 1vw; cursor: pointer; color: red;"
...
async def apply_effect_endpoint(filename: str):
    try:
        ...
        original_image_path, output_path = apply_effect(image_path)
        ...
        <html>
            ...
            <body>
                ...
                <img src="/uploads/{original_image_path.name}" alt="Effect Applied" style="max-width: 50%;">
                <br>
                <h3>Original Image:</h3>
                <img src="/uploads/{output_path.name}" alt="Original Image" style="max-width: 50%;">
            </body>
        </html>
        """)
    except Exception as e:
